...
public class YtSubtitlesLoader {

    private static final Logger log = LoggerFactory.getLogger(YtSubtitlesLoader.class);

    private final YtLinkHelper ytLinkHelper;
    private final YtDlpProcessBuilderCreator ytDlpProcessBuilderCreator;

    public YtSubtitlesLoader(YtLinkHelper ytLinkHelper, YtDlpProcessBuilderCreator ytDlpProcessBuilderCreator) {
    ...
    public YtSubtitles load(String dirtyLink) {
        ...
        try {
            Pair<String, SubtitlesLang> subtitlesContentAndLangPair = loadSubtitles(cleanedLink);
            String subtitlesContent = subtitlesContentAndLangPair.getLeft();
            SubtitlesLang lang = subtitlesContentAndLangPair.getRight();

            List<String> strings = List.of(subtitlesContent.split("\n"));
    ...
    /**
     * Загружаем любые доступные субтитры (русские или английские, обычные или автогенерированные)
     * Пытаемся в следующем порядке:
     * 1. Русские обычные субтитры
     * 2. Русские автогенерированные субтитры
     * 3. Английские обычные субтитры
     * 4. Английские автогенерированные субтитры
     *
     * @param videoUrl ссылка на видео
     * @return пара, где первый параметр - содержимое загруженных субтитров, второй параметр - язык
     */
    private Pair<String, SubtitlesLang> loadSubtitles(String videoUrl) {
        Pair<SubtitlesLang, Boolean> preferred = selectPreferredSubtitles(videoUrl)
        ...
        String subtitles = tryDownloadSubtitles(videoUrl, lang, isAuto);
        if (StringUtils.isEmpty(subtitles)) {
            throw new YtLoadingException("48ae", "Не удалось загрузить субтитры для указанного видео");
        }

        return Pair.of(subtitles, lang);
    }
    ...
    private Optional<Pair<SubtitlesLang, Boolean>> selectPreferredSubtitles(String videoUrl) {
        ...
        try {
            ...
            if (ruManualVtt) return Optional.of(Pair.of(SubtitlesLang.RU, false));
            if (ruAutoVtt)   return Optional.of(Pair.of(SubtitlesLang.RU, true));
            if (enManualVtt) return Optional.of(Pair.of(SubtitlesLang.EN, false));
            if (enAutoVtt)   return Optional.of(Pair.of(SubtitlesLang.EN, true));
    ...
    private String tryDownloadSubtitles(String videoUrl, SubtitlesLang lang, boolean isAutoSubs) {
        try {
            ...
            Pair<ProcessBuilder, java.nio.file.Path> builderAndPath = ytDlpProcessBuilderCreator.createProcessBuilder(
                videoUrl, lang.getLang(), isAutoSubs
            );
            ProcessBuilder processBuilder = builderAndPath.getLeft();
            java.nio.file.Path outputPath = builderAndPath.getRight();
            ...
            String subtitles = java.nio.file.Files.readString(outputPath).trim();
